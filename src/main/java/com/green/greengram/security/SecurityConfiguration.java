...
public class SecurityConfiguration {
    private final JwtAuthenticationFilter jwtAuthenticationFilter; //DI(@Component로 빈등록 되어있음)
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity/*라이브러리에 빈등록 되어있음+스프링이 DI*/) throws Exception{
        ...
        return httpSecurity.sessionManagement(session -> /*람다식*/
                ...
                .authorizeHttpRequests(auth->auth.requestMatchers(
                        ...
                        .anyRequest()/*어떤 요청이든*/.authenticated()/*권한이 있는 자만 사용할 수 있다*/)//순서가 중요 url을 설정한 부분이 any 뒤에 있으면 적용 X
